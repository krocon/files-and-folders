...
// const logger = new Logger('FileActionControllerSpec');
...
describe('FileActionController', () => {
  ...
  describe('mkdir: test/specbox', () => {
    it('should create directory and return no error', (done) => {
      const appController = app.get<FileActionController>(FileActionController);

      const target = new FileItem(testDir, 'specbox', '');
      target.isDir = true;
      const filePara = new FilePara(null, target, 'mkdir');

      appController.onDo(filePara).then(res => {
      ...
      // expect(appController.onDo(filePara)).resolves.toMatchObject({ error: "", cmd: "mkdir" });
    });
  });
  ...
  describe('remove: test/specbox', () => {
    it('should delete directory and return no error', (done) => {
      const appController = app.get<FileActionController>(FileActionController);
  ...
  describe('unpack: demo.zip', () => {
    it("should unpack zip file-content demo.zip", async () => {
      ...
      return await expect(appController.onDo(filePara)).resolves.toEqual(20);
    });
  });

  describe('unpacklist: demo.zip', () => {
    it('list files of demo.zip', (done) => {
      const appController = app.get<FileActionController>(FileActionController);
      const filePara = new DirPara(testDir + '/demo.zip', 'tab'+126, true);
      appController.unpacklist(filePara).then(res => {
  ...
  describe('remove: test/demo', () => {
    it('should delete directory and return no error', async () => {
      ...
      return await expect(appController.onDo(filePara)).resolves.toMatchObject(expected);
    });
  });
